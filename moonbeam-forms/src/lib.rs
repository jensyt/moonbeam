//! HTML form and multipart support for Moonbeam.
//!
//! Provides the [`Form`] extractor which handles both `application/x-www-form-urlencoded`
//! and `multipart/form-data` transparently.
...
/// `Content-Type`.
pub struct Form<'a>(FormType<'a>);

impl<'a> Form<'a> {
	/// Returns an iterator over values for a specific field name.
	pub fn find<'b>(&self, name: &'b str) -> FormIterator<'a, 'b, '_> {
		match &self.0 {
			FormType::URLEncoded(p) => FormIterator(FormIteratorType::URLEncoded(p.find(name))),
			FormType::Multipart(m) => FormIterator(FormIteratorType::MultiPart(m.find(name))),
		}
	}
}
...
impl<'a> TryFrom<Request<'_, 'a>> for Form<'a> {
	...
	fn try_from(req: Request<'_, 'a>) -> Result<Self, Self::Error> {
		if req.method.eq_ignore_ascii_case("get") {
			return Ok(Form(FormType::URLEncoded(req.params())));
		...
		if content_type.starts_with(b"application/x-www-form-urlencoded") {
			...
			Ok(Form(FormType::URLEncoded(Params::new(body))))
			...
			Ok(Form(FormType::Multipart(Multipart::new(
				boundary, req.body,
			))))
		} else {
...
enum FormIteratorType<'a, 'b, 'c> {
	URLEncoded(ParamIter<'c, 'b>),
	MultiPart(PartIter<'a, 'b, 'c>),
}

/// An iterator over form fields with a specific name.
pub struct FormIterator<'a, 'b, 'c>(FormIteratorType<'a, 'b, 'c>);
...
impl<'a> Iterator for FormIterator<'_, '_, 'a> {
	...
	fn next(&mut self) -> Option<Self::Item> {
		match &mut self.0 {
			FormIteratorType::URLEncoded(p) => p.next().map(FormData::Text),
			FormIteratorType::MultiPart(p) => p.next(),
		}
	}
}
...
impl<'a> Multipart<'a> {
	fn new(boundary: &'a [u8], body: &'a [u8]) -> Multipart<'a> {
		let mut parts = vec![];
	...
	fn find<'b>(&self, name: &'b str) -> PartIter<'a, 'b, '_> {
		PartIter {
